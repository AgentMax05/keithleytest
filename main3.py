...
# 1) Create the controller with Kp, Ki, Kd, and setpoint

def temperature_pid():
    pid = PID(Kp=1.2, Ki=0.01, Kd=0.005, setpoint=TARGET_TEMPERATURE)

    # 2) (Optional) Limit output to actuator range, e.g., 0–5 V
    pid.output_limits = (0, 5)

    # 3) In your control loop:
    while True:
        measured = read_temperature()
        control_signal = pid(measured)  # PID computes Kp*err + Ki*∫err + Kd*(derr/dt)
        set_voltage(control_signal)
        time.sleep(0.5)  # sample time
